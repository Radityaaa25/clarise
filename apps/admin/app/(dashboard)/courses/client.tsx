...
export function CoursesClient({ initialCourses }: { initialCourses: CourseItem[] }) {
  ...
  const handleDelete = async (id: string, title: string) => {
    if (!confirm(`Yakin ingin menghapus kursus "${title}" secara permanen?`))
      return;
    setLoadingId(id);
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-muted-foreground font-bold mt-2">
            Review flagged courses, approve new content, or unpublish
            violations.
          </p>
        </div>
              ...
              >
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-3">
                    <a
                      href={`${apiUrl}/course/${course.slug}`}
                      target="_blank"
                      rel="noreferrer"
                      className="neo-btn bg-card text-foreground p-2 hover:bg-muted"
                      title="Preview"
                    >
