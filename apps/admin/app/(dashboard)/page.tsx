        ...
        {stats.map((stat, i) => (
          <div key={i} className="neo-card p-6">
            <div className="flex justify-between items-start mb-4">
              <div
                className={`p-3 border-2 border-border neo-shadow-sm ${stat.bg}`}
              >
            ...
            <div className="flex items-end gap-2 mt-1">
              ...
              <span
                className={`text-sm font-bold mb-1 ${stat.change.includes("+") ? "text-primary" : stat.change.includes("Needed") ? "text-destructive" : "text-muted-foreground"}`}
              >
                {stat.change}
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 neo-card min-h-[400px] flex flex-col items-center justify-center p-6">
          <p className="text-muted-foreground font-bold border-2 border-dashed border-border p-8 text-center w-full h-full flex items-center justify-center">
            {premiumUsers > 0
              ? "Revenue Chart akan aktif saat Duitku terhubung"
              : "Belum ada transaksi premium bulan ini."}
          </p>
        </div>
        <div className="neo-card p-6 min-h-[400px] flex flex-col">
          <h3 className="text-xl font-black text-foreground mb-6 uppercase border-b-2 border-border pb-2">
            Recent Users
          </h3>
