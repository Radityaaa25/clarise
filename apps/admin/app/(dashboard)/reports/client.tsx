              ...
              <tr key={report.id} className="hover:bg-muted/50 transition-colors bg-background">
                ...
                <td className="px-6 py-4 border-r-2 border-border">
                  <span className={`px-3 py-1 text-xs font-black uppercase border-2 border-border neo-shadow-sm ${
                    report.status === "PENDING" ? "text-destructive-foreground bg-destructive" : 
                    report.status === "RESOLVED" ? "text-primary-foreground bg-primary" : "bg-card text-foreground"
                  }`}>
                    {report.status}
                  </span>
                </td>
                <td className="px-6 py-4 text-right">
                  {report.status === "PENDING" && (
                    <div className="flex justify-end gap-3">
                      ...
                      >
                        {loadingId === report.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
                      </button>
                      <button 
