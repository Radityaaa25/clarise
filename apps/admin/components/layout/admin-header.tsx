...
export function AdminHeader() {
  const { data, mutate } = useSWR("http://localhost:3000/api/notifications", fetcher);
        ...
        <div className="relative">
          ...
          >
            <div className="flex items-center justify-between mb-4 border-b-2 border-border pb-2">
              <h3 className="font-bold text-sm text-foreground">Notifikasi Sistem</h3>
              {unreadCount > 0 && (
                <span className="text-xs font-bold bg-destructive text-destructive-foreground px-2 py-0.5 border-2 border-border">{unreadCount} Baru</span>
              )}
            </div>
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto">
              {notifications.length > 0 ? (
                notifications.map((notif: any) => (
                  <div key={notif.id} className={`flex gap-3 p-3 border-2 border-border cursor-pointer neo-shadow-sm ${notif.isRead ? 'bg-background' : 'bg-accent'}`}>
                    {!notif.isRead && <div className="h-3 w-3 mt-1 bg-destructive border-2 border-border shrink-0" />}
                    <div className={notif.isRead ? 'ml-3' : ''}>
                      ...
                      <p className="text-[10px] font-bold mt-2">{new Date(notif.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                ))
              ) : (
            ...
            {unreadCount > 0 && (
              <div className="mt-4 pt-2 border-t-2 border-border text-center">
                <button onMouseDown={handleMarkAllRead} className="text-sm font-bold hover:underline">
                  Tandai semua dibaca
                </button>
              </div>
            )}
