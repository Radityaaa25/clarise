...
export function ArrowButton({ children, onClick, href, className = "", disabled }: {
  ...
  return (
    <Tag href={href} onClick={onClick} disabled={disabled} className={`ab ${className} ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}>
      ...
      <span className="ab-top" aria-hidden="true">{children}</span>
      <span className="ab-bot" aria-hidden="true">{children}</span>
