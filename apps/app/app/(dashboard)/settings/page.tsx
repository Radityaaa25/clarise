...
import { User, Mail, Bell, Shield, CreditCard, ChevronRight, Tag, Loader2 } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const { data: reminderData, mutate: mutateReminder } = useSWR("/api/user/reminder", fetcher);
  ...
  const handleRedeemVoucher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setRedeemMessage({ type: "error", text: data.error || "Gagal klaim voucher" });
      }
    } catch (err) {
            ...
            <>
              ...
              <div className="rounded-2xl border border-hairline shadow-sm bg-canvas dark:bg-void-elevated p-6 md:p-8">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8">
                  ...
                  <div>
                    <div className="flex gap-3 mb-2">
                      <button disabled className="opacity-50 px-4 py-2 rounded-lg bg-surface-soft dark:bg-white/10 text-ink dark:text-white text-sm font-semibold transition-colors border border-hairline dark:border-white/10">
                        Ubah Foto (Via Clerk)
                      </button>
                    </div>
                ...
                <div className="mt-8 flex justify-end">
                  <button onClick={handleUpdateProfile} disabled={isSavingProfile} className="btn-outline-capsule disabled:opacity-50">
          ...
          {activeTab === "keamanan" && (
            <div className="rounded-2xl border border-hairline shadow-sm bg-canvas dark:bg-void-elevated p-6 md:p-8">
              ...
              <div className="space-y-6">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-ink dark:text-white/90">Ubah Password</label>
                  <p className="text-sm text-muted mb-4">Ubah password akunmu melalui penyedia layanan autentikasi (Clerk).</p>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 rounded-xl border border-hairline dark:border-white/10 bg-surface-soft/50 dark:bg-void">
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" className="sr-only peer" checked={isPromoEnabled} readOnly />
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border border-hairline dark:border-white/10 bg-surface-soft/50 dark:bg-void">
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" className="sr-only peer" checked={isReminderEnabled} onChange={handleToggleReminder} />
                    <div className="w-11 h-6 bg-hairline peer-focus:outline-none rounded-full peer dark:bg-white/20 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-core-blue"></div>
                  </label>
                </div>
              </div>
          ...
          {activeTab === "langganan" && (
            <div className="rounded-2xl border border-hairline shadow-sm bg-canvas dark:bg-void-elevated p-6 md:p-8">
              ...
              <div className="p-6 rounded-xl border border-core-blue/20 bg-core-blue/5 flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-bold text-ink dark:text-white mb-2">Paket Dasar (Free)</h3>
                <p className="text-sm text-muted mb-6">Kamu saat ini menggunakan paket gratis. Upgrade untuk menikmati akses tanpa batas.</p>
              ...
              <div className="mt-8 p-6 rounded-xl border border-hairline bg-surface-soft/30 dark:bg-void">
                ...
                <p className="text-sm text-muted mb-4">Punya kode promo atau akses trial? Masukkan kodenya di bawah ini.</p>
                ...
                {redeemMessage && (
                  <div className={`mt-4 p-3 rounded-lg text-sm font-bold border ${
