...
export async function POST(req: Request) {
  try {
    ...
    const accountAgeDays = (new Date().getTime() - new Date(currentUser.createdAt).getTime()) / (1000 * 60 * 60 * 24);
    if (accountAgeDays > 7) {
      return NextResponse.json({ error: "Kode referral hanya bisa digunakan oleh akun baru (maksimal 7 hari sejak daftar)" }, { status: 400 });
    }
    ...
    if (referrer.id === currentUser.id) {
      return NextResponse.json({ error: "Anda tidak bisa menggunakan kode referral Anda sendiri" }, { status: 400 });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (giveReward) {
        ...
        if (existingSub && existingSub.status === "ACTIVE" && existingSub.endDate) {
        ...
        await tx.subscription.upsert({
          ...
          update: {
            status: "ACTIVE",
            plan: existingSub?.plan === "FREE" ? "PREMIUM_TRIAL" : existingSub?.plan,
            endDate: newEndDate
          }
        });
      }
